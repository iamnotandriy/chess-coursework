...
const showLeaderboardModal = ref(false); // leaderboard as modal window
...
const goToGame = (mode) => {
  ...
  if (mode === 'standard' && !currentUser.value) {
...
const menuItems = [
  ...
  { id: 'leaderboard', label: 'LEADERBOARD', desc: 'TOP PLAYERS', subItems: [
    { label: 'VIEW RANKINGS', desc: 'SEASON 1 TOP 10', action: openLeaderboard }
  ]},
...
<template>
  <div class="home-layout">
    ...
    <div class="user-badge glass-panel" v-if="currentUser" @click="showProfileModal = true">
      ...
      <div class="u-info"><span class="u-name">{{currentUser.username}}</span><div class="badges-row-mini"><span class="u-rating">{{currentUser.rating}} ELO</span><span v-if="currentUser.rank" class="rank-badge small" :class="getRankClass(currentUser.rank)">#{{currentUser.rank}}</span></div></div>
    </div>
    ...
    <div class="menu-center-wrapper">
      ...
      <div class="footer">v2.0.0 Final</div>
    </div>
...
.auth-card, .profile-card, .settings-modal { padding: 50px 40px; width: 450px; border-radius: 16px; display: flex; flex-direction: column; gap: 20px; text-align: center; transform: skewX(-2deg); }
.profile-card { width: 550px; }
.lb-modal-card { height: 600px; width: 500px; }
...
.lb-rank { width: 40px; font-weight: 900; font-size: 0.9rem; color: #888; text-align: center; flex-shrink: 0; }
...
.profile-actions { display: flex; justify-content: center; gap: 30px; margin-top: 20px; }
