...
const opponentInfo = ref({ username: 'Opponent', rating: '???', avatar: '', id: null, rank: null });
const currentUserInfo = ref({ username: 'You', rating: 1200, avatar: '', rank: null });
...
onMounted(async () => {
  ...
  if (myStats) {
    currentUserInfo.value = { username: myStats.username, rating: myStats.rating, avatar: myStats.avatar, rank: myStats.rank };
  }
...
onMounted(() => {
  ...
  socket.on('game_start', (data) => {
    ...
    if(data.opponent) {
        opponentInfo.value = { ...data.opponent };
        if (data.opponent.id && !data.opponent.isPrivate) {
            authService.getPublicProfile(data.opponent.id).then(p => { if(p.rank) opponentInfo.value.rank = p.rank; });
        }
    }
    if(data.mode) gameMode.value = data.mode;
    
    if(data.fen) { game.load(data.fen); rawHistory.value = data.history || []; } else { game.reset(); rawHistory.value = []; }
...
<template>
  <div class="layout">
    ...
    <Transition name="slide-in"><div v-if="showOpponentProfile && opponentProfileData" class="side-panel-overlay" @click.self="showOpponentProfile = false"><div class="side-profile glass-panel"><div class="panel-header"><h3>PLAYER INFO</h3><button class="btn-icon close-p" @click="showOpponentProfile = false">✕</button></div><div class="profile-content"><img v-if="opponentProfileData.avatar" :src="opponentProfileData.avatar" class="big-avatar-img"><div v-else class="big-avatar"></div><h2 class="profile-name">{{ opponentProfileData.username }}</h2><div class="badges-row"><span class="elo-badge">{{ opponentProfileData.rating }} ELO</span><span v-if="opponentProfileData.rank" class="rank-badge" :class="getRankClass(opponentProfileData.rank)">#{{opponentProfileData.rank}}</span></div><div class="bio-section"><span class="label">BIO</span><p class="bio-text">{{ opponentProfileData.bio || 'No bio provided.' }}</p></div><div class="stats-grid"><div class="stat-box"><span class="stat-val">{{ opponentProfileData.wins }}</span><span class="stat-label">WINS</span></div><div class="stat-box"><span class="stat-val">{{ opponentProfileData.matches }}</span><span class="stat-label">GAMES</span></div></div></div></div></div></Transition>
        ...
        <aside class="sidebar glass-panel">
          <div class="player-card" :class="{offline:opponentDisconnected, clickable: canViewProfile}" @click="viewOpponent">
            ...
            <div class="player-details">
              ...
              <div class="p-stats-row"><span class="p-status">{{opponentDisconnected?'OFFLINE':opponentInfo.rating+' ELO'}}</span><span v-if="opponentInfo.rank" class="rank-badge small" :class="getRankClass(opponentInfo.rank)">#{{opponentInfo.rank}}</span></div>
            </div>
          ...
          <div class="player-card" :class="{'active-turn':isMyTurn&&!gameOver}">
            ...
            <div class="player-details">
              ...
              <div class="p-stats-row"><span class="p-status">{{ currentUserInfo.rating }} ELO</span><span v-if="currentUserInfo.rank" class="rank-badge small" :class="getRankClass(currentUserInfo.rank)">#{{currentUserInfo.rank}}</span></div>
            </div>
            <div v-if="isMyTurn&&!gameOver" class="turn-marker"></div>
          </div>
        </aside>
...
.panel-header h3 { margin: 0; letter-spacing: 2px; }
.close-p { font-size: 1.5rem; color: var(--danger); background: none; border: none; cursor: pointer; }
...
.big-avatar { width: 120px; height: 120px; background: #333; border: 3px solid var(--accent); border-radius: 12px; margin: 0 auto; }
